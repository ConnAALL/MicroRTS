 ...
 * with a value for output
 * If it's a neuron, it has a list of its incoming input signals Use an
 * activation count to avoid flushing
 */
public class NNode extends Neat {
	...
	Vector<Link> incoming = new Vector<Link>(1, 0);

	/** A list of pointers to links carrying this node's signal */
	Vector<Link> outgoing = new Vector<Link>(1, 0);
	...
	/**
	 * activation value of node at time t-1; Holds the previous step's activation
	 * for recurrency
	 */
	double last_activation;

	/**
	 * activation value of node at time t-2 Holds the activation before the previous
	 * step's
	 */
	 ...
	 * node (genotype) to a real node (fenotype)
	...
	public Vector<Link> getIncoming() {
		return incoming;
	}

	public void setIncoming(Vector<Link> incoming) {
		this.incoming = incoming;
	}

	public Vector<Link> getOutgoing() {
		return outgoing;
	}

	public void setOutgoing(Vector<Link> outgoing) {
	...
	public NNode(String xline, Vector<Trait> traits) {
