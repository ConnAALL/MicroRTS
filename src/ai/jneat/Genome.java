...
public class Genome extends Neat {
	 ...
	 * the list of Genes provide an evolutionary history of innovation and
	 * link-building
	 */
	Vector genes;
		...
		if (found) {
			...
			while (!done) {
				...
				if (!itr_innovation.hasNext()) {
					...
					// Create the new gene
					new_gene = new Gene((Trait) traits.elementAt(traitnum), new_weight, thenode1, thenode2, do_recur,
							curr_innov, new_weight);
					// Add the innovation
					pop.innovations
							.add(new Innovation(thenode1.node_id, thenode2.node_id, curr_innov, new_weight, traitnum));
					done = true;
				...
				// OTHERWISE, match the innovation in the innovs list
					...
					if ((_innov.innovation_type == NeatConstant.NEWLINK)
						...
						new_gene = new Gene((Trait) traits.elementAt(_innov.new_traitnum), _innov.new_weight, thenode1,
								thenode2, do_recur, _innov.innovation_num1, 0);
						done = true;
					}
				}
			}
	...
	public boolean mutate_add_node(Population pop) {
		...
		while (!done) {
			...
			if (!itr_innovation.hasNext()) {
				...
				pop.innovations.add(new Innovation(in_node.node_id, out_node.node_id, gene_innov1, gene_innov2,
			...
			// end for new innovation case
				...
				if ((_innov.innovation_type == NeatConstant.NEWNODE)
					...
					newgene1 = new Gene(traitptr, 1.0, in_node, new_node, thelink.is_recurrent, _innov.innovation_num1,
							0);
				...
				for (row = 1; row <= totalnodes; row++) {
					...
					if ((cmp[innov_number] && (col > i))
						...
						if (create_gene) {
							...
							newgene = new Gene(newtrait, new_weight, in_node, out_node, flag_recurrent, innov_number,
									new_weight);
