...
public class Species extends Neat {
	 ...
	 */
	Vector<Organism> organisms = new Vector<>(1, 0);
	...
	public void adjust_fitness() {
		...
		// floor is the largest (closest to positive infinity) double value that is not
		// greater
	...
	*/
	public boolean reproduce(int generation, Population pop, Vector<Species> sorted_species) {
		...
		for (count = 0; count < expected_offspring; count++) {
			...
			//
			// If we have a super_champ (Population champion), finish off some special
			// clones
			//
			// System.out.print("\n verifica select....");
			if (thechamp.super_champ_offspring > 0) {
				...
				if ((thechamp.super_champ_offspring) == 1) {
					if (thechamp.pop_champ) {
						// System.out.print("\n The new org baby's (champion) genome is :
						// "+baby.genome.getGenome_id());
			...
			// Otherwise we should mate
				...
				} else if (NeatRoutine.randfloat() < (Neat.p_mate_multipoint_avg_prob
						/ (Neat.p_mate_multipoint_avg_prob + Neat.p_mate_singlepoint_prob))) {
					// System.out.print("\n mate multipoint_avg baby: ");
					new_genome = mom.genome.mate_multipoint_avg(_dad.genome, count, mom.orig_fitness,
							_dad.orig_fitness);
				} else {
				...
				if ((NeatRoutine.randfloat() > Neat.p_mate_only_prob) || (_dad.genome.genome_id == mom.genome.genome_id)
			...
			if (pop.species.isEmpty()) {
				...
				// looop in all species.... (each species is a Vector of organism...) of
				// population 'pop'
