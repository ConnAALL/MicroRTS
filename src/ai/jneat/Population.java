...
public class Population extends Neat {
	...
	public void epoch(int generation) {
		...
		// sorted species has all species ordered : the species with orig_fitness
		// maximum is first
		...
		if (((Organism) curspecies.organisms.firstElement()).orig_fitness > highest_fitness) {
			...
			// System.out.print("\n Are passed "+ highest_last_changed+ " generations from
			// last population fitness record: "+ highest_fitness);
			rep1.append("\n    are passed " + highest_last_changed
					+ " generations from last population fitness record: " + highest_fitness);
		}
		...
		if (highest_last_changed >= Neat.p_dropoff_age + 5) {
			...
			if (itr_specie.hasNext()) {
				...
				_specie.expected_offspring = half_pop;
				_specie.age_of_last_improvement = _specie.age;
				// at this moment the offpring is terminated : the remainder species has 0
				// offspring!
				while (itr_specie.hasNext()) {
					_specie = ((Species) itr_specie.next());
					_specie.expected_offspring = 0;
			...
			if (Neat.p_babies_stolen > 0) {
				...
				for (int j = sorted_species.size() - 1; (j >= 0) && (stolen_babies < NUM_STOLEN); j--) {
					...
					// System.out.print("\n Analisis SPECIE #"+j+" (size =
					// "+_specie.organisms.size()+" )");
		...
		itr_organism = organisms.iterator();
		Vector<Organism> vdel_org = new Vector<Organism>(organisms.size());
		...
		while (itr_organism.hasNext()) {
			...
			if (_organism.eliminate) {
				...
				vdel_org.add(_organism);
			}
		}
		// eliminate organism from master list
		for (int i = 0; i < vdel_org.size(); i++) {
			Organism _organism = (Organism) vdel_org.elementAt(i);
		...
		Vector<Species> vdel_spec = new Vector<Species>(species.size());
		orgcount = 0;
		...
		// System.out.print("\n the number of species can be eliminated is
		// "+vdel.size());
		// eliminate species marked from master list
		for (int i = 0; i < vdel_spec.size(); i++) {
			_specie = (Species) vdel_spec.elementAt(i);
		...
		while (itr_organism.hasNext()) {
			...
			if (_organism.pop_champ_child) {
				// System.out.print("\n At end of reproduction cycle, the child of the pop champ
				// is: ");
				break;
			}
		}
	...
	public void speciate() {
		...
		species = new Vector<Species>(1, 0);
		...
		while (itr_organism.hasNext()) {
			...
			if (species.isEmpty()) {
				...
				while (!done && itr_specie.hasNext()) {
					...
					// compare _organism-esimo('_organism') with first organism in current
					// specie('compare_org')
